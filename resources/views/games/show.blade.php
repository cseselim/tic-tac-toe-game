    ...
    @if ($isFullGameField || $prepareData['gameOver'])
        ...
        @if ($prepareData['playerWinner'])
            <div class="text-center f-size-2">
                Congratulation  "{{ strtoupper($game::getPlayerTypes($prepareData['playerWinner'])) }}"
            </div>
        @else
            <div class="text-center f-size-2">
                It is a Tie!
    ...
    @else
        ...
        @if ($prepareData['playerType'] && $prepareData['playerType'] === $firstPlayerType)
            <div class="d-flex align-items-center justify-content-center">
                <div class="mb-2 player">Your Turn - <span>{{ $game::getPlayerTypes($secondPlayerType) }}</span></div>
            </div>
        @endif

        @if ($prepareData['playerType'] && $prepareData['playerType'] === $secondPlayerType)
            <div class="d-flex align-items-center justify-content-center">
                <div class="mb-2 player">Your Turn - <span>{{ $game::getPlayerTypes($firstPlayerType) }}</span></div>
            </div>
        @endif
